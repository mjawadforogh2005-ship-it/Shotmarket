...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const createButton = document.querySelector(".create-album-btn");
  let selectedPhotos = [];
  ...
  function updatePhotoCount() {
    ...
    photoCountLabel.textContent = `${count} photo${count === 1 ? "" : "s"}`;
  }
  ...
  if (albumForm) {
    albumForm.addEventListener("submit", async (event) => {
      ...
      if (!user) {
        alert("Please log in before creating an album.");
      ...
      const privacyElement = document.querySelector(
        'input[name="privacy"]:checked',
      );
      const privacy = privacyElement ? privacyElement.value : "private";
      if (!albumName) {
      ...
      try {
        ...
        currentGalleryToken = galleryToken;
        ...
        if (albumError) {
          ...
          throw new Error("Could not create album: " + albumError.message);
        }
        console.log("Album created:", album);
        const photoRecords = [];
        for (const photo of selectedPhotos) {
          const safeFileName = photo.file.name.replace(/[^a-zA-Z0-9._-]/g, "_");
          const storagePath = `${user.id}/${album.id}/${Date.now()}-${safeFileName}`;
        ...
        const galleryURL = getGalleryURL(
          album.id,
          album.gallery_token || galleryToken,
        );
        generateQRCode(album.id, album.gallery_token || galleryToken);
        const { error: qrError } = await supabase
          ...
          .eq("id", album.id);
        if (qrError) {
          console.warn("QR URL could not be saved:", qrError);
        }
        if (successMessage) {
        ...
        sessionStorage.setItem("shotmarket_current_album", album.id);
        console.log("ShotMarket album completed:", album.id);
        console.log("Gallery URL:", galleryURL);
        alert("Album created and photos uploaded successfully!");
      } catch (error) {
        console.error("ShotMarket upload error:", error);
        alert(
          error.message || "Something went wrong while creating the album.",
        );
      } finally {
        if (createButton) {
  ...
  function generateQRCode(albumId, galleryToken) {
    if (!qrContainer) {
      console.warn("QR container not found.");
      return;
    }
    qrContainer.innerHTML = "";
    const galleryURL = getGalleryURL(albumId, galleryToken);
    ...
    if (downloadQrButton) {
      ...
      downloadQrButton.onclick = downloadQRCode;
    }
    console.log("QR Code URL:", galleryURL);
  }
  function getGalleryURL(albumId, galleryToken) {
    const basePath =
  ...
  function downloadQRCode() {
    ...
    link.download = "shotmarket-qr-code.png";
