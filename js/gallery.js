...
const SUPABASE_KEY = "sb_publishable_16S4x_HPLxfsUk1RTgR4Qw_gnvlyqD_";
const supabase = createClient(SUPABASE_URL, SUPABASE_KEY);
document.addEventListener("DOMContentLoaded", async () => {
  console.log("ShotMarket Gallery System Loaded 🚀");
  ...
  const downloadPurchasedBtn = document.getElementById("downloadPurchasedBtn");
  ...
  let galleryToken = urlParams.get("token");
  console.log("Gallery Album ID:", albumId);
  console.log("Gallery token present:", Boolean(galleryToken));
  ...
  async function requestPurchasedDownloads() {
    try {
      ...
      if (!session) {
        alert("Please log in to download your purchased photos.");
        return;
      }
      ...
      if (!result.downloads || result.downloads.length === 0) {
        throw new Error("No purchased downloads are available.");
      }
      ...
      alert(error.message || "Could not download your photos.");
    }
  }
  async function loadGallery() {
    try {
      const functionUrl = `${SUPABASE_URL}/functions/v1/gallery-access`;
      ...
      console.log("Secure gallery loaded:", album);
      console.log("Secure photos loaded:", photos);
      displayAlbum();
      displayPhotos();
    } catch (error) {
  ...
  function displayAlbum() {
    ...
    if (albumTitle) {
      albumTitle.textContent = album.name || "Untitled Album";
    }
    if (albumDescription) {
    ...
    if (albumDate) {
      albumDate.textContent = formatDate(album.event_date);
    }
    if (albumLocation) {
      albumLocation.textContent = album.location || "—";
    }
    if (albumPhotoCount) {
      albumPhotoCount.textContent = `${photos.length} photo${
        photos.length === 1 ? "" : "s"
      }`;
    }
  }
  function displayPhotos() {
    ...
    if (!photos.length) {
      ...
      if (purchaseBar) {
        ...
        purchaseBar.classList.remove("active");
      }
      return;
    }
    if (emptyState) {
    ...
    if (purchaseBar) {
      ...
      purchaseBar.classList.toggle("active", selectedPhotos.length > 0);
    }
    photos.forEach((photo, index) => {
      ...
      card.innerHTML = `
                        <div class="photo-image-wrapper">
                            <img
                                src="${escapeHTML(photo.displayUrl)}"
                                alt="${escapeHTML(
                                  photo.file_name || "ShotMarket Photo",
                                )}"
                                loading="lazy"
                            >
  ...
  function attachPhotoEvents() {
    ...
    const previewButtons = photoGrid.querySelectorAll(".preview-photo-btn");
    ...
    const imageWrappers = photoGrid.querySelectorAll(".photo-image-wrapper");
  ...
  function updateSelection() {
    ...
    if (purchaseCount) {
      purchaseCount.textContent = count;
    }
    if (purchaseBar) {
      purchaseBar.classList.toggle("active", count > 0);
    }
    if (continuePaymentBtn) {
      continuePaymentBtn.disabled = count === 0;
    }
  ...
  if (selectAllBtn) {
    selectAllBtn.addEventListener("click", () => {
      if (selectedPhotos.length === photos.length) {
        ...
        selectedPhotos = photos.map((photo) => photo.id);
        const checkboxes = photoGrid.querySelectorAll(".photo-checkbox");
        checkboxes.forEach((checkbox) => {
  ...
  if (continuePaymentBtn) {
    continuePaymentBtn.addEventListener("click", () => {
      ...
      sessionStorage.setItem("shotmarket_current_album", albumId);
      sessionStorage.setItem("shotmarket_gallery_token", galleryToken);
      window.location.href =
        "payment.html?album=" +
        encodeURIComponent(albumId) +
        "&token=" +
        encodeURIComponent(galleryToken);
    });
  }
  if (downloadPurchasedBtn) {
    downloadPurchasedBtn.addEventListener("click", requestPurchasedDownloads);
  }
  ...
  function updateLightbox() {
    ...
    if (lightboxImage) {
      ...
      lightboxImage.alt = photo.file_name || "ShotMarket Photo";
    }
    if (lightboxCounter) {
  ...
  async function showGalleryFinder() {
    ...
    if (form) {
      form.addEventListener("submit", (e) => {
        e.preventDefault();
        const albumIdValue = document
          .getElementById("albumIdInput")
          .value.trim();
